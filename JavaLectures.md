...
# Лекция 1 "Вводная" (07.09.2020)
	...
	3. XML веб-сервисы, REST веб-сервисы
	4. БД + Java
	5. Методы авторизации и аутентификации
...
# Лекция 4 "Класслоадеры"(28.09.2020)
	Класс для загрузки классов. Загруженный класс обладает связью с загрузившим его класслоадером. То есть один и тот же класс, загруженный разными класслоадерами, являются разными классами.
	Это позволяет загружать классы во время выполнения приложения без необходимости его перекомпиляции.
	Система безопасности Java также модет быть настроена для конкретного класслоадера. В protection domain можно указать список разрешений/запретов на класслоадер. Аналогичное можно задать для конкретного класса.
