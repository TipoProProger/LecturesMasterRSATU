...
# Лекция 9 "Основные моменты приложения" (02.11.2020)
Сервлет - RFC2616  
URL - читай стандарт  
...
# Лекция 10 "ORM" (09.11.2020)
> Object to relation bridge
Позволяет преобразовывать структуру реляционной базы данных в классовую структуру. Также позволяет производить обратное преобразование.  
ORM реализаций существует много. Поэтому интереснее рассматривать стандарт. Для Java это стандарт JPA JSR 238.  
## Entity
Entity классом может быть только обычный класс. Не интерфейс и не внутренний класс
Entity класс не может быть final. Это требуется для его наследования и дальнейшей инструментации его.  
POJO класс должен обладать get\set методами для атрибутов.
Serializable - по факту пуст
Для соединения сущностей друг с другом используются специальные аннотации:
- @OneToOne
- @OneToMany
- @ManyToMany  
Выносить бизнес логику на эти поля плохая идея. Вместо этого есть модель вид-контроллер.

## Фазы
1. Deploy  
- Сканируются классы
- Находятся классы с Entity
- Проверка модели (аннотации + отсутствие противоречий)
- Генерация классов-контейнеров
- Просматривается конфигурация, выделяются PersistenceUnit (единица deploy'а) и класс размещаются по ним. 
- Создаются объекты EntityManager (опциональный пункт)  
2. Выполнение
- Инжекция EntityManager
- Соединение с БД
- Работа  

> Один-к-одномк лучше не использовать
Многие-к-одному и один-ко-многим
Владелец связи с левой стороны один-ко-многим. Хранится список или мапа или сет для хранения связи.

Вид связи может быть LAZY или EAGER. LAZY - подгружает только по требованию.  
mappedBy - имя поля в связанной сущности. Что-то вроде внешнего ключа.
joinColumn - ссылка на поле, на которое ссылаемся.  
Можно наследовать Entity друг от друга. Это может быть выгодно, если предок может ссылаться сначала на одну сущность, потом на другую.  

# Лекция 11 "?" (16.11.2020)
Существует проблема при обработке большого количества запросов. Например, необходимо вставить в базу 10^5 записей. Делать это через insert медленно, так как на каждый insert будет формироваться пакет, затем обрабатываться. Выгоднее использовать bulk. Это позволит работать с данными как с блоком, что разгоняет работу.

Блокировки:
- Оптимистическая  
Разрешается читать. Также добавляется поле "номер изменения" или "версия записи". Записываем туда текущее состояние. При попытке внести изменения, явно указывается текущая версия. В случае несовпадения, изменение не будет применено. Это позволяет защититься от непредвиденной перезаписи.
- Пессимистическая  
Блокировка без возможности чтения и записи

detach - отсоединяет класс от entity manager. Последующие обращений от класса к менеджеру не будет.

Виды запросов:
1. Нативные (SQL)
2. JPQL (SQL, сделанный через объектную структуру (классы))
3. Criteria API (запрос пишется в коде через методы). По идее это лучше, так как проверка происходит на этапе компиляции. НЕ ЯВЛЯЕТСЯ СЕРЕБРЯНОЙ ПУЛЕЙ
4. NamedQuery (резервируем имя и пишем sql код для выполнения)
2 и 3 создает +- одинаковые запросы

Хранимые процедуры
Например решают проблему генерации id. Систему требуется сделать под разные СУБД.  
Позволяют решать стандартные проблемы: применени до вставки или после.  
Второй вариант, если данных очень много и большая часть тратится на передачу данных. То есть операция простая, но данных много. Тогда можно сделать хранимую процедуру, чтобы избавится от передачи данных от базы к обработчику. Применять нужно правильно, чтобы без бессмысленных излишеств.  
Транзация (@Transactional) сохраняется в рамках потока. Есть специальный менеджер транзакций.  

Можно заранее наделать графов для выбора способа загрузки связи.  

Метамодель hibernate. Может быть статическая, можно сгенерировать. 
